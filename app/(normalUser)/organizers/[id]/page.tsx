...
async function getOrganizerProfile(id: string) {
    try {
        ...
        const organizer = await User.findById(id).select(
            "name email imageUrl coverImageUrl description role followers"
        ).lean();
...
async function getOrganizerEvents(organizerId: string) {
    try {
        ...
        console.error("Error fetching organizer events:", JSON.stringify(error, null, 2));
        return [];
    }
}
                        ...
                        <div className="flex-1 text-white md:text-slate-900 md:pt-20">
                            ...
                            <div className="flex flex-wrap text-white/90 md:text-slate-500 font-medium gap-4">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span>{organizer.role === "organizer" ? "Organization" : "User"}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
