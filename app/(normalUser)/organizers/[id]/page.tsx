...
export default async function OrganizerProfilePage({
  ...
  return (
    <main className="min-h-[calc(100vh-56px)] bg-slate-50 dark:bg-transparent pb-12">
      {/* Cover Image */}
        ...
        <AnimatedPageHeader>
          <div className="flex flex-col md:flex-row items-end md:items-center gap-6 mb-8">
            ...
            <div className="relative shrink-0">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-white dark:border-slate-800 shadow-2xl overflow-hidden bg-white dark:bg-slate-900">
                {organizer.imageUrl ? (
                  <Image
                  ...
                  <div className="w-full h-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-4xl font-bold text-indigo-500 dark:text-indigo-400">
            ...
            {/* Info */}
            <div className="flex-1 text-white md:text-slate-900 dark:md:text-white md:pt-20">
              <h1 className="text-3xl font-black tracking-tight drop-shadow-lg md:drop-shadow-none mb-2">
                {organizer.name}
              </h1>
              <div className="flex flex-wrap text-white/90 md:text-slate-500 dark:md:text-slate-400 font-medium gap-4">
            ...
            <div className="md:pt-20">
              {currentUser && currentUser.userId === organizer._id ? (
                <Link
                  href="/profile"
                  className="inline-flex items-center gap-2 px-6 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700/80 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors shadow-sm"
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <AnimatedCard delay={0.3}>
              <div>
                <h2 className="text-xl font-black text-slate-900 dark:text-white mb-6 flex items-center gap-2 px-1">
                  Events by {organizer.name}
                  <span className="bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-xs py-1 px-3 rounded-full ml-auto">
                ...
                {events.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {events.map((event: any) => (
                      <Link
                        key={event._id}
                        href={`/home/${event._id}`}
                        className="group block bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden hover:shadow-lg hover:border-indigo-100 dark:hover:border-indigo-900/50 transition-all duration-300"
                      >
                        {/* Card Image */}
                        <div className="relative h-40 w-full bg-slate-100 dark:bg-slate-800 overflow-hidden">
                          {event.coverImageUrl ? (
                            ...
                            <div className="absolute inset-0 flex items-center justify-center bg-indigo-50 dark:bg-indigo-900/20 text-indigo-300 dark:text-indigo-700">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900 rounded-2xl p-8 text-center text-slate-400 dark:text-slate-500">
          ...
          <div className="lg:col-span-1">
            ...
            <AnimatedCard delay={0.4}>
              <div className="bg-indigo-50/50 dark:bg-indigo-900/20 rounded-3xl p-6 border border-indigo-50 dark:border-indigo-900/30 sticky top-24">
                <p className="text-center text-indigo-900/60 dark:text-indigo-300/60 text-sm font-medium">
