...
async function getEventAttendees(eventId: string) {
    await connectDb();
    try {
        const attendees = await User.find({ eventId })
            .sort({ createdAt: -1 })
            .lean();
        return attendees.map((a: any) => {
            ...
            const fullName = `${firstName} ${lastName}`.trim() || (a.attendeeName || ""); // Fallback for old records
            return {
                id: a._id.toString(),
                name: fullName,
                                ...
                                <div className="p-6">
                                    {attendees.length === 0 ? (
                                        <div className="text-center py-12">
                                            ...
                                            <h3 className="text-lg font-semibold text-slate-900 mb-2">No attendees yet</h3>
